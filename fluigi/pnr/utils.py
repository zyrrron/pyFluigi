...
def size_nodes(parchmint_device: Device) -> None:
    if parchmint_device.graph is None:
        raise ValueError("MINT Device is None")
    for component in parchmint_device.components:
        if component.entity == "NODE":
            # Look at the connections
            nbers = parchmint_device.graph.edges(component.ID)
            gedges = list(nbers)
            if len(gedges) > 0:
                gedge = gedges[0]
            else:
            ...
            connection = parchmint_device.graph.get_edge_data(gedge[0], gedge[1])[0]["connection_ref"]
            channel_width = connection.params.get_param("channelWidth")
            component.xspan = channel_width
            component.yspan = channel_width


def check_ref_and_assign_port(source_ref, connection, parchmint_device: Device, global_port_assign_map):
    source_name = source_ref.component
    source = parchmint_device.get_component(source_name)
...
def assign_component_ports(parchmint_device: Device) -> None:
    ...
    for connection in parchmint_device.connections:
        ...
        for sink_ref in connection.sinks:
            check_ref_and_assign_port(sink_ref, connection, parchmint_device, global_port_assign_map)


def reduce_device_size(parchmint_device: Device, design_padding: int) -> None:
    ...
    xspan = parchmint_device.xspan
    yspan = parchmint_device.yspan

    xspan = max_x + 2 * design_padding
    yspan = max_y + 2 * design_padding

    parchmint_device.xspan = xspan
    parchmint_device.yspan = yspan
