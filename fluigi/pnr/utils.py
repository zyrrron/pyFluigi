...
def size_nodes(parchmint_device: Device) -> None:
    ...
    for component in parchmint_device.components:
        if component.entity == "NODE":
            ...
            connection = parchmint_device.graph.get_edge_data(gedge[0], gedge[1])[0][
                "connection_ref"
            ]
...
def check_ref_and_assign_port(
    source_ref, connection, parchmint_device: Device, global_port_assign_map
):
...
def assign_component_ports(parchmint_device: Device) -> None:
    ...
    for connection in parchmint_device.connections:
        ...
        check_ref_and_assign_port(
            source_ref, connection, parchmint_device, global_port_assign_map
        )
        for sink_ref in connection.sinks:
            check_ref_and_assign_port(
                sink_ref, connection, parchmint_device, global_port_assign_map
            )
